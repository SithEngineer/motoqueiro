...
final class Mapper {
  ...
  static Func6<Cursor, List<GpsPoint>, List<HeartRatePoint>, List<TriDimenPoint>, List<TriDimenPoint>, List<TriDimenPoint>, Ride>
      CURSOR_TO_RIDE;
  ...
  static Func1<Cursor, TriDimenPoint> CURSOR_TO_GYROSCOPE_POINT;
  ...
  static {
    ...
    CURSOR_TO_GYROSCOPE_POINT = (cursor) -> {
      long timestamp = cursor.getLong(
          cursor.getColumnIndex(RidePersistenceContract.GyroscopeEntry.COLUMN_TIMESTAMP));
      float xx = cursor.getFloat(
          cursor.getColumnIndex(RidePersistenceContract.GyroscopeEntry.COLUMN_XX));
      float yy = cursor.getFloat(
          cursor.getColumnIndex(RidePersistenceContract.GyroscopeEntry.COLUMN_YY));
      float zz = cursor.getFloat(
          cursor.getColumnIndex(RidePersistenceContract.GyroscopeEntry.COLUMN_ZZ));
      return new TriDimenPoint(xx, yy, zz, timestamp);
    };
    ...
    CURSOR_TO_RIDE =
        (cursor, gpsPoints, heartRatePoints, accelPoints, gravityPoints, gyroscopePoints) -> {
          ...
          long finalTimestamp = cursor.getLong(cursor.getColumnIndex(
          ...
          boolean completed = cursor.getInt(
              cursor.getColumnIndex(RidePersistenceContract.RideEntry.COLUMN_COMPLETED))
              == 1;

          boolean synced = cursor.getInt(
              cursor.getColumnIndex(RidePersistenceContract.RideEntry.COLUMN_SYNCED))
              == 1;

          Ride part =
              new Ride(id, name, initialTimestamp, finalTimestamp, completed, synced);
          ...
          part.setGyroscopeCaptures(gyroscopePoints);
          return part;
        };
