...
public class RxBluetoothGattCallback extends BluetoothGattCallback {
  ...
  @Override public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
    if (newState == BluetoothProfile.STATE_CONNECTED) {
      ...
      gattEventBehaviorSubject.onNext(new GattEvent(GattEvent.Type.CONNECT, gatt, status));
      return;
    }
    gattEventBehaviorSubject.onNext(new GattEvent(GattEvent.Type.DISCONNECT, gatt, status));
  }

  @Override public void onServicesDiscovered(BluetoothGatt gatt, int status) {
    if (status == BluetoothGatt.GATT_SUCCESS) {
      gattEventBehaviorSubject.onNext(new GattEvent(GattEvent.Type.SERVICE_DISCOVER, gatt, status));
    }
  }
