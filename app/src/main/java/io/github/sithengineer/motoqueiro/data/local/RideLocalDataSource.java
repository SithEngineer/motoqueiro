...
public class RideLocalDataSource implements RideDataSource {
  ...
  @Override public Single<Long> saveHeartRateData(String rideId, HeartRatePoint point) {
    return Single.fromCallable(() -> {
      ...
      values.put(RidePersistenceContract.HeartRateEntry.COLUMN_BPM, point.getBeatsPerMinute());
  ...
  @Override public Completable saveRide(RidePart ride) {
    return Completable.fromCallable(() -> checkNotNull(ride))
        ...
        .flatMapCompletable(transaction -> Completable.fromAction(() -> {
          ...
          // save ride
          databaseHelper.insert(RidePersistenceContract.RideEntry.TABLE_NAME, values, SQLiteDatabase.CONFLICT_REPLACE);
        })
  ...
  @Override public Observable<RidePart> getRide(String rideId) {
    ...
    QueryObservable ridePartQuery = databaseHelper.createQuery(RidePersistenceContract.HeartRateEntry.TABLE_NAME, sql, rideId);
    ...
    return ridePartQuery.flatMap(query -> {
      ...
      if (cursor == null || !cursor.moveToFirst()) {
        return Observable.error(new IllegalStateException("Unable to run database query."));
      }
      return Observable.zip(gpsPointsObservable, accelPointsObservable, gravityPointsObservable, heartRatePointsObservable,
  ...
  @Override public Completable markCompleted(String rideId) {
    return Completable.fromCallable(() -> checkNotNull(rideId))
        ...
        .flatMapCompletable(transaction -> Completable.fromAction(() -> {
          ...
          values.put(RidePersistenceContract.RideEntry.COLUMN_END_TIMESTAMP, new Date().getTime());
